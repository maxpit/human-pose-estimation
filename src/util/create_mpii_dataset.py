...
def add_to_tfrecord(anno, img_id, img_dir, coder, writer, is_train):
    ...
    print("image name", anno_info.image.name)
    ...
    for joints, scale, pos in people:
        ...
        # Encode image:
        image_data_scaled = tf.image.encode_jpeg(image_scaled)

        utils.write_example(height, width, center, joints_scaled, image_path, image_data_scaled,
                            seg_data_scaled, writer)
    ...
    while i < len(img_inds):
        ...
        with tf.python_io.TFRecordWriter(tf_filename) as writer:
            ...
            while i < len(img_inds) and num_ppl < num_shards:
                ...
                num_ppl += add_to_tfrecord(anno, img_inds[i], img_dir,
                                           writer, is_train)
                i += 1
